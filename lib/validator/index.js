module.exports = {
    ...
    async validateFields(app, fields, parent, errors, fieldname) {
        for (let field of fields) {
            ...
            fieldname = fieldname ? `${fieldname}[${field.name}]` : field.name;
            ...
            if (field.options && field.options.rules) {
                await this.validateField(app, field, value, errors, fieldname);
            }

            if (field.type == 'object' && field.sub) {
                await this.validateFields(app, field.sub, value, errors, fieldname);
            }

            if (field.type == 'array' && field.sub && field.sub[0] && field.sub[0].sub) {
                if (Array.isArray(value) && value.length) {
                    for (let i = 0; i < value.length; i++) {
                        await this.validateFields(app, field.sub[0].sub, value[i], errors, `${fieldname}[${i}]`);
                    }
                } else {
                    await this.validateFields(app, field.sub[0].sub, null, errors, `${fieldname}[0]`);
                }
            }
        }
    },
