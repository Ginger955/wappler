...
function cy(y, h, meta) {
    if (typeof y == 'string') {
        switch (y) {
            ...
            default:
                if (/%$/.test(y)) {
                    y = (meta.height - h) * parseFloat(x) / 100;
                }
        }
    }
...
module.exports = {
    ...
    crop: async function(options) {
        ...
        let width = this.parseRequired(cw(options.width), 'number', 'image.crop: width is required.');
        let height = this.parseRequired(ch(options.height), 'number', 'image.crop: height is required.');
        let left = this.parseRequired(cx(options.x, width, sharp.metadata), 'number', 'image.crop: x is required.');
        let top = this.parseRequired(cy(options.y, height, sharp.metadata), 'number', 'image.crop: y is required.');
    ...
    watermark: async function(options) {
        ...
        let left = this.parseRequired(cx(options.x, metadata.width, sharp.metadata), 'number', 'image.watermark: x is required.');
        let top = this.parseRequired(cx(options.y, metadata.height, sharp.metadata), 'number', 'image.watermark: y is required.');
