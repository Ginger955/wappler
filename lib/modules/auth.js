module.exports = {
    ...
    provider: async function(options, name) {
        if (!name) throw new Error('auth.provider has no name.');
        const provider = this.setAuthProvider(name, options);
    ...
    validate: async function(options) {
        const provider = this.getAuthProvider(this.parseRequired(options.provider, 'string', 'auth.validate: provider is required.'));
    ...
    login: async function(options) {
        const provider = this.getAuthProvider(this.parseRequired(options.provider, 'string', 'auth.login: provider is required.'));
    ...
    logout: async function(options) {
        const provider = this.getAuthProvider(this.parseRequired(options.provider, 'string', 'auth.logout: provider is required.'));

        return provider.logout();
    },

    restrict: async function(options) {
        const provider = this.getAuthProvider(this.parseRequired(options.provider, 'string', 'auth.restrict: provider is required.'));
